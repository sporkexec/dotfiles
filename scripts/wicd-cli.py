...
#       This adds options to set up ad-hoc mode. I should probably send in a
#       patch, the different dev branches are a little confusing though... I'll
#       do it soon, really! -- Jacob Courtneay <jacob@sporkexec.com>
...
parser.add_option('--adhoc', '-A', default=False, action='store_true')
parser.add_option('--adhoc-essid', '-E', default='My_Adhoc_Network')
parser.add_option('--adhoc-channel', '-C', type='int', default=3)
parser.add_option('--adhoc-ip', '-I', default='169.254.12.10')
parser.add_option('--adhoc-key', '-K', default=False)
...
if options.wireless and options.adhoc:
	wireless.CreateAdHocNetwork(options.adhoc_essid,
			str(options.adhoc_channel),
			options.adhoc_ip, 'WEP',
			options.adhoc_key,
			options.adhoc_key is not False, False)
	op_performed = True
